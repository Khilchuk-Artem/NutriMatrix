...
using RecommendationService.Application.Models.Genetic;
...
namespace RecommendationService.Api.Services.RecommendationService
{
    public partial class RecipeRecommendationService : IRecipeRecommendationService
    {
        ...
        public async Task<RecommendationResponseDto> GetRecommendationAsync(RecommendationRequestDto dto)
        {
            ...
            foreach (var rc in bestCombo)
            {
                var recipe = rc.Recipe;
                var amount = rc.Amount;
                var servings = recipe.Servings;

                foreach (var kvp in recipe.NutrientAmounts)
                {
                    var nutrientId = kvp.Key;
                    var perServingAmt = kvp.Value;

                    if (!dto.NutritionalGoals.ContainsKey(nutrientId))
                        continue;

                    float contribution = perServingAmt * amount / servings;

                    if (aggregated.ContainsKey(nutrientId))
                    {
                        aggregated[nutrientId] += contribution;
                    }
                    else
                    {
                        aggregated[nutrientId] = contribution;
