...
export class EditPrivateRecipeComponent implements OnInit {
  ...
  onHoverFood(food: FoodShortcutDTO) {
    const nutrients = this.authService.getUser()?.nutrientsToTrack.filter(r=>r.isActive).map(r=>r.nutrientId) || [];
    this.hoverCancel$.next();
    this.hoveredFood = food;
    this.hoveredFood.nutrients = this.hoveredFood.nutrients.filter(r=>nutrients.includes(r.nutrientId));
