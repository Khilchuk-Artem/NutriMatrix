...
@Component({
  selector: 'app-add-public-recipe',
  imports: [
    ReactiveFormsModule,
    NgIf,
    NgForOf,
  ...
  standalone: true,
  styleUrl: './add-public-recipe.component.css'
})
export class AddPublicRecipeComponent implements OnInit {
  ...
  onSubmit() {
    if (this.recipeForm.valid && this.addedIngredients.length > 0) {
      ...
      const dto: CreateOrUpdateRecipeDto = {
        ...
        } as IngredientMeasureDto)),
        nutrients: this.getTotalNutrients().map(n => ({
          nutrientId: n.nutrientId,
          amount: n.amount
        } as NutrientAmountDto))
      };
      this.recipeService.createRecipe(dto).subscribe({
        next: (response) => {
          this.router.navigate(['/app/recipes/public', response.id]);
        },
        error: (err) => {
